...
use crate::Error;
use crate::read_util::{ByteOrder, Readable, ReadableRead, Writable, WritableWrite};
use crate::Result;
...
impl Readable for Request {
    fn read(stream: &mut std::io::BufReader<impl Read>, order: &ByteOrder) -> Result<Self> {
        ...
        match opcode {
            ...
            21 => Ok(Request::RotateProperties(stream.read_value(order)?)),
            22 => Ok(Request::ListProperties(stream.read_value(order)?)),
            23 => Ok(Request::SetSelectionOwner(stream.read_value(order)?)),
            24 => Ok(Request::GetSelectionOwner(stream.read_value(order)?)),
            25 => Ok(Request::ConvertSelection(stream.read_value(order)?)),
            26 => Ok(Request::SendEvent(stream.read_value(order)?)),
            27 => Ok(Request::GrabPointer(stream.read_value(order)?)),
            28 => Ok(Request::UngrabPointer(stream.read_value(order)?)),
            29 => Ok(Request::GrabButton(stream.read_value(order)?)),
            30 => Ok(Request::UngrabButton(stream.read_value(order)?)),
            31 => Ok(Request::ChangeActivePointerGrab(stream.read_value(order)?)),
            32 => Ok(Request::GrabKeyboard(stream.read_value(order)?)),
            33 => Ok(Request::UngrabKeyboard(stream.read_value(order)?)),
            34 => Ok(Request::GrabKey(stream.read_value(order)?)),
            35 => Ok(Request::UngrabKey(stream.read_value(order)?)),
            36 => Ok(Request::AllowEvents(stream.read_value(order)?)),
            37 => Ok(Request::GrabServer(stream.read_value(order)?)),
            38 => Ok(Request::UngrabServer(stream.read_value(order)?)),
            39 => Ok(Request::QueryPointer(stream.read_value(order)?)),
            40 => Ok(Request::GetMotionEvents(stream.read_value(order)?)),
            41 => Ok(Request::TranslateCoordinates(stream.read_value(order)?)),
            42 => Ok(Request::WarpPointer(stream.read_value(order)?)),
            43 => Ok(Request::SetInputFocus(stream.read_value(order)?)),
            44 => Ok(Request::GetInputFocus(stream.read_value(order)?)),
            45 => Ok(Request::QueryKeymap(stream.read_value(order)?)),
            46 => Ok(Request::OpenFont(stream.read_value(order)?)),
            47 => Ok(Request::CloseFont(stream.read_value(order)?)),
            48 => Ok(Request::QueryFont(stream.read_value(order)?)),
            49 => Ok(Request::QueryTextExtents(stream.read_value(order)?)),
            50 => Ok(Request::ListFonts(stream.read_value(order)?)),
            51 => Ok(Request::ListFontsWithInfo(stream.read_value(order)?)),
            52 => Ok(Request::SetFontPath(stream.read_value(order)?)),
            53 => Ok(Request::GetFontPath(stream.read_value(order)?)),
            54 => Ok(Request::CreatePixmap(stream.read_value(order)?)),
            55 => Ok(Request::FreePixmap(stream.read_value(order)?)),
            56 => Ok(Request::CreateGC(stream.read_value(order)?)),
            57 => Ok(Request::ChangeGC(stream.read_value(order)?)),
            58 => Ok(Request::CopyGC(stream.read_value(order)?)),
            59 => Ok(Request::SetDashes(stream.read_value(order)?)),
            60 => Ok(Request::SetClipRectangles(stream.read_value(order)?)),
            61 => Ok(Request::FreeGC(stream.read_value(order)?)),
            62 => Ok(Request::ClearArea(stream.read_value(order)?)),
            63 => Ok(Request::CopyArea(stream.read_value(order)?)),
            64 => Ok(Request::CopyPlane(stream.read_value(order)?)),
            65 => Ok(Request::PolyPoint(stream.read_value(order)?)),
            66 => Ok(Request::PolyLine(stream.read_value(order)?)),
            67 => Ok(Request::PolySegment(stream.read_value(order)?)),
            68 => Ok(Request::PolyRectangle(stream.read_value(order)?)),
            69 => Ok(Request::PolyArc(stream.read_value(order)?)),
            70 => Ok(Request::FillPoly(stream.read_value(order)?)),
            71 => Ok(Request::PolyFillRectangle(stream.read_value(order)?)),
            72 => Ok(Request::PolyFillArc(stream.read_value(order)?)),
            73 => Ok(Request::PutImage(stream.read_value(order)?)),
            74 => Ok(Request::GetImage(stream.read_value(order)?)),
            75 => Ok(Request::PolyText8(stream.read_value(order)?)),
            76 => Ok(Request::PolyText16(stream.read_value(order)?)),
            77 => Ok(Request::ImageText8(stream.read_value(order)?)),
            78 => Ok(Request::ImageText16(stream.read_value(order)?)),
            79 => Ok(Request::CreateColormap(stream.read_value(order)?)),
            80 => Ok(Request::FreeColormap(stream.read_value(order)?)),
            81 => Ok(Request::CopyColormapAndFree(stream.read_value(order)?)),
            82 => Ok(Request::InstallColormap(stream.read_value(order)?)),
            83 => Ok(Request::UninstallColormap(stream.read_value(order)?)),
            84 => Ok(Request::ListInstalledColormaps(stream.read_value(order)?)),
            85 => Ok(Request::AllocColor(stream.read_value(order)?)),
            86 => Ok(Request::AllocNamedColor(stream.read_value(order)?)),
            87 => Ok(Request::AllocColorCells(stream.read_value(order)?)),
            88 => Ok(Request::AllocColorPlanes(stream.read_value(order)?)),
            89 => Ok(Request::FreeColors(stream.read_value(order)?)),
            90 => Ok(Request::StoreColors(stream.read_value(order)?)),
            91 => Ok(Request::StoreNamedColor(stream.read_value(order)?)),
            92 => Ok(Request::QueryColors(stream.read_value(order)?)),
            93 => Ok(Request::LookupColor(stream.read_value(order)?)),
            94 => Ok(Request::CreateCursor(stream.read_value(order)?)),
            95 => Ok(Request::CreateGlyphCursor(stream.read_value(order)?)),
            96 => Ok(Request::FreeCursor(stream.read_value(order)?)),
            97 => Ok(Request::RecolorCursor(stream.read_value(order)?)),
            98 => Ok(Request::QueryBestSize(stream.read_value(order)?)),
            99 => Ok(Request::QueryExtension(stream.read_value(order)?)),
            100 => Ok(Request::ListExtensions(stream.read_value(order)?)),
            101 => Ok(Request::SetModifierMapping(stream.read_value(order)?)),
            102 => Ok(Request::GetModifierMapping(stream.read_value(order)?)),
            103 => Ok(Request::ChangeKeyboardMapping(stream.read_value(order)?)),
            104 => Ok(Request::GetKeyboardMapping(stream.read_value(order)?)),
            105 => Ok(Request::ChangeKeyboardControl(stream.read_value(order)?)),
            106 => Ok(Request::GetKeyboardControl(stream.read_value(order)?)),
            107 => Ok(Request::Bell(stream.read_value(order)?)),
            108 => Ok(Request::SetPointerMapping(stream.read_value(order)?)),
            109 => Ok(Request::GetPointerMapping(stream.read_value(order)?)),
            110 => Ok(Request::ChangePointerControl(stream.read_value(order)?)),
            111 => Ok(Request::GetPointerControl(stream.read_value(order)?)),
            112 => Ok(Request::SetScreenSaver(stream.read_value(order)?)),
            113 => Ok(Request::GetScreenSaver(stream.read_value(order)?)),
            114 => Ok(Request::ForceScreenSaver(stream.read_value(order)?)),
            115 => Ok(Request::ChangeHosts(stream.read_value(order)?)),
            116 => Ok(Request::ListHosts(stream.read_value(order)?)),
            117 => Ok(Request::SetAccessControl(stream.read_value(order)?)),
            118 => Ok(Request::SetCloseDownMode(stream.read_value(order)?)),
            119 => Ok(Request::KillClient(stream.read_value(order)?)),
